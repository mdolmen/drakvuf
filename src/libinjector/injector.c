...
struct injector
{
    ...
    addr_t sc_addr;
    addr_t write_proc_mem, msgbox, glerror;
    uint32_t status;
...
bool pass_inputs(struct injector* injector, drakvuf_trap_info_t* info)
{
    ...
    else
    {
        ...
        else if (INJECT_METHOD_SHELLCODE == injector->method && STATUS_NULL == injector->status)
        {
            ...
            uint64_t null64 = 0;
            uint64_t mem_size = 4096; // TODO : replace with size of SC
            uint64_t allocation_type = MEM_COMMIT | MEM_RESERVE;
            uint64_t protect = PAGE_EXECUTE_READWRITE;

            init_argument(&args[0], ARGUMENT_INT, sizeof(uint64_t), (void*)null64, 0);
            init_argument(&args[1], ARGUMENT_INT, sizeof(uint64_t), (void*)mem_size, 0);
        ...
        else if (INJECT_METHOD_SHELLCODE == injector->method && STATUS_ALLOC_OK == injector->status)
        {
            ...
            // TODO : do that somewhere else, before calling VirtualAlloc
            if ( !(fp = fopen(injector->target_file, "rb")) )
                goto err;

            shellcode = g_malloc0(sizeof(char) * mem_size);

            // Get payload from the file
            tmp = shellcode;
            ...
            PRINT_DEBUG("(debug) size of shellcode : %lu\n", payload_size);

            /*
            unsigned char shellcode[94] =
            "\x49\xbe\x2f\x85\xe0\xc0\xcc\x9e\xa1\xd9\x54\x5a\xdb\xd6\x66"
            "\x81\xe2\x30\xf5\x4d\x31\xff\x48\x0f\xae\x02\x41\xb7\x06\x4c"
            "\x8b\x4a\x08\x49\xff\xcf\x4f\x31\x74\xf9\x22\x4d\x85\xff\x75"
            "\xf3\x67\x06\x0c\xd8\x0b\xda\x85\xdd\x2e\x85\xe0\xc0\x0b\x9a"
            "\x85\xdf\x2f\x85\xe0\x4b\xc8\xba\x2a\x95\x0b\x81\xe3\x08\x47"
            "\x5f\xe9\x5a\xeb\x9d\x23\x3f\x33\x61\x5e\x26\xd0\x7a\x1f\x3f"
            "\x33\x61\x5e\x26";
            */
            ...
            init_argument(&args[2], ARGUMENT_STRING, payload_size, (void*)shellcode, 0);
            init_argument(&args[3], ARGUMENT_INT, sizeof(uint64_t), (void*)payload_size, 0);
        ...
        // TODO : modify setup_stack to handle better function with no arguments
        else if (INJECT_METHOD_SHELLCODE == injector->method && STATUS_WRITE_OK == injector->status)
        {
            // Setup the stack with empty parameters to allocate homing space
        ...
        else if (INJECT_METHOD_SHELLCODE == injector->method && GET_LAST_ERROR == injector->status)
        {
            struct argument args[4] = { {0} };
            uint64_t null64 = 0;
...
event_response_t injector_int3_cb(drakvuf_t drakvuf, drakvuf_trap_info_t* info)
{
    ...
    if ( !injector->is32bit && info->regs->rip == injector->bp.breakpoint.addr && STATUS_WRITE_OK == injector->status)
    {
        ...
        // DEBUG
        unsigned char buffer[94] = { '\0' };
        ctx.addr = injector->sc_addr;
        if ( VMI_SUCCESS == vmi_read(injector->vmi, &ctx, 94, buffer, NULL) )
        {
            printf("(debug) vmi_read of RIP: ");
            for (int i = 0; i < 94; i++)
                printf("0x%02x ", buffer[i]);
            putchar('\n');
        }
        else
            PRINT_DEBUG("(debug) vmi_read failed");
        // END_DEBUG
...
int injector_start_app(drakvuf_t drakvuf, vmi_pid_t pid, uint32_t tid, const char* file, const char* cwd, injection_method_t method, output_format_t format)
{
    ...
    else if (INJECT_METHOD_SHELLCODE == method)
    {
        ...
        injector.msgbox = drakvuf_exportsym_to_va(injector.drakvuf, eprocess_base, lib, fun);
        if (!injector.msgbox)
        {
            PRINT_DEBUG("Failed to get address of %s!%s\n", lib, fun);
            goto done;
        ...
        lib = "kernel32.dll";
        fun = "GetLastError";
        injector.glerror = drakvuf_exportsym_to_va(injector.drakvuf, eprocess_base, lib, fun);
        if (!injector.msgbox)
        {
            PRINT_DEBUG("Failed to get address of %s!%s\n", lib, fun);
            goto done;
